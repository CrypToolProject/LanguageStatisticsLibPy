...
class LanguageStatistics:
    ...
    def create_grams(language_code, language_statistics_directory, grams_type, use_spaces):
        if grams_type == GramsType.Unigrams:
            ...
            raise ValueError("Unsupported grams type")
    ...
    @staticmethod
    def get_supported_grams_types(language, language_statistics_directory, use_spaces=False):
        types_list = []
        for i in range(1, 6):
            filename = f"{language}-{i}gram{'-sp' if use_spaces else ''}.bin"
            if os.path.exists(os.path.join(language_statistics_directory, filename)):
                types_list.append(LanguageStatistics.get_grams_type_by_length(i))
        return types_list
    ...
    def load_word_tree(language_code, language_statistics_directory):
        ...
        with gzip.open(filename, 'rb') as filestream:
            word_tree = WordTree.deserialize(filestream)
        return word_tree
